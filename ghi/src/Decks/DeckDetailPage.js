...
function DeckDetailPage() {
    ...
    const getDeck = async() =>{
        ...
        if (deckData) {
            const timedDeck = helper.createTimesAgos(deckData)
            setDeck(timedDeck);
            ...
            for (let card_number of deckData.cards) {
                const mainDeckCard = cardData.cards.find(card => card.card_number === card_number)
                mainList.push(mainDeckCard)
            }
            setMainList(mainList)

            for (let card_number of deckData.pluck) {
                const pluckDeckCard = cardData.cards.find(card => card.card_number === card_number)
