from queries.card_comps import (
    CardTypeQueries,
    CardTypeIn,
    CardTypeOut,
    CardTypesAll,
...
def get_all_card_types(queries: CardTypeQueries = Depends()):
    return {
        "card_types": queries.get_all_card_types(),
    }
...
def get_card_type(
    ...
    record = queries.get_card_type(card_type_id)
    if record is None:
...
def create_card_type(
    ...
    return queries.create_card_type(card_type_in)
...
def update_card_type(
    ...
    record = queries.update_card_type(card_type_id, card_type_in)
    if record is None:
        response.status_code = 404
    else:
        return record
...
def delete_card_type(
    ...
    queries.delete_card_type(card_type_id)
    return True
